...
class RuleBasedFallDetector:
    ...
    def __init__(
        self,
        low_fps: bool = True,  # 低帧率（5–10 fps）或高帧率（≥25 fps）
        model_path: str = "./pose_demo/yolo11n-pose.pt",
        conf_thresh: float = 0.2,        # 低于此置信度的关键点忽略
        angle_upright_max: float = 30.0, # (°) 躯干-竖直夹角阈值
        angle_lying_min: float = 65.0,   # (°) 判定平躺
        ratio_upright_min: float = 1.2,  # h/w，大于此认为竖直
        ratio_lying_max: float = 0.5,    # h/w，小于此认为横躺
        v_speed_thresh: float = 0.25,    # 髋关节归一化纵向速度阈值
        lying_frames_confirm: int = 8,   # 连续帧数≥N 认定 FALL
        buffer_size: int = 30,           # 最近 N 帧缓存
        ...
        self._buf       = deque(maxlen=buffer_size)  # 每项: (mid_hip_y_norm, ts)
        self._lying_cnt = 0
